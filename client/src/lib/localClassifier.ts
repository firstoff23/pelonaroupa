...
export async function runLocalYAMNet(
  audioBlob: Blob,
): Promise<{ state: string; confidence: number; emoji: string }> {
  try {
    ...
    try {
      ...
      console.warn(
        "Could not load YAMNet from TFHub, falling back to local heuristic classifier:",
        err,
      );
    ...
    } else if (catPurrScore > 0.2) {
      return { state: "relaxed", confidence: catPurrScore, emoji: "⚪" };
    } else if (dogBarkScore > 0.2 || catMeowScore > 0.2) {
      return {
        state: "excitement",
        confidence: Math.max(dogBarkScore, catMeowScore),
        emoji: "🟢",
      };
    }
  } catch (e) {
...
function runLocalHeuristicClassifier(signal: Float32Array): {
  state: string;
  confidence: number;
  emoji: string;
} {
  ...
  if (zcr < 0.08) {
    ...
    return Math.random() > 0.4
      ? { state: "relaxed", confidence: 0.7 + Math.random() * 0.2, emoji: "⚪" }
      : {
          state: "hunger",
    ...
    return Math.random() > 0.5
      ? { state: "alert", confidence: 0.7 + Math.random() * 0.2, emoji: "🔵" }
      : {
          state: "attention",
