...
def parse_optional_split(value: Any) -> str | None:
    if value is None:
        return None
...
def row_from_mapping(row: dict[str, Any]) -> ManifestRow:
    return ManifestRow(
        sample_id=str(
            ...
            or Path(str(row.get("audio_path", "audio"))).stem
        ),
        audio_path=str(row.get("audio_path") or row.get("file_name") or ""),
        source_dataset=str(
            row.get("source_dataset") or row.get("dataset") or "unknown"
        ),
        species=normalize_label(row.get("species")) or "unknown",
        animal_id=(
            normalize_identifier(row.get("animal_id"))
            or normalize_identifier(row.get("dog_id"))
            or normalize_identifier(row.get("cat_id"))
            or "unknown"
        ),
        ...
        split=parse_optional_split(row.get("split")),
    )
...
def validate_rows(rows: list[ManifestRow]) -> list[str]:
    ...
    for idx, row in enumerate(rows):
        if row.sample_id in seen:
            errors.append(f"duplicate sample_id at row {idx}: {row.sample_id}")
        seen.add(row.sample_id)
        if not row.audio_path:
            errors.append(f"missing audio_path: {row.sample_id}")
        if row.species not in {"dog", "cat", "dog_cat", "unknown"}:
            errors.append(f"invalid species={row.species!r}: {row.sample_id}")
        if not row.animal_id or row.animal_id.lower() in {"unknown", "nan", "none", "null"}:
            errors.append(f"missing animal_id: {row.sample_id}")
        if row.duration_s is not None and not isinstance(row.duration_s, (int, float)):
            errors.append(f"invalid duration_s={row.duration_s!r}: {row.sample_id}")
        if row.sample_rate is not None and not isinstance(row.sample_rate, int):
            errors.append(f"invalid sample_rate={row.sample_rate!r}: {row.sample_id}")
        if row.arousal is not None and not isinstance(row.arousal, (int, float)):
            errors.append(f"invalid arousal={row.arousal!r}: {row.sample_id}")
        if row.split is not None and row.split not in VALID_SPLITS:
            errors.append(f"invalid split={row.split!r}: {row.sample_id}")
    return errors
