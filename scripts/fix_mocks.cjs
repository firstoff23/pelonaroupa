...
files.forEach((f) => {
  ...
  const targetRegex =
    /single:\s*vi\.fn\(\)\.mockImplementation\(\(\)\s*=>\s*\{\s*if\s*\(table\s*===\s*"users"\)\s*\{\s*return\s*Promise\.resolve\(\{\s*data:\s*\{\s*id:\s*1\s*\},\s*error:\s*null\s*\}\);\s*\}\s*return\s*Promise\.resolve\(\{\s*data:\s*null,\s*error:\s*null\s*\}\);\s*\}\),/g;
